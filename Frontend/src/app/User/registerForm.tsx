...
export default function RegisterForm() {
	...
	async function handleSubmit(_previousState: unknown, formData: FormData) {
		...
		if (!email) return { emailError: "Email is required.", previousInputData };
		else if (!username)
			...
			return {
				passwordError: `Password must meet the following requirements:\n- ${passwordErrors.join("\n- ")}`,
				previousInputData,
			};
		...
		<div className="fixed inset-0 bg-teal-800 flex justify-center items-center">
			<div className="bg-white mt-20 p-8 rounded-lg shadow-lg w-96">
				...
				<form action={action}>
					...
					/>
					{data?.usernameError && (
						<p className="text-red-500 text-sm mb-2">{data?.usernameError}</p>
					)}
